...
typedef int32_t(STDCALL *read_write_func) (Il2CppIntPtr buffer, int32_t length, Il2CppIntPtr gchandle);
...
static int32_t write_to_managed(ZStream *stream)
{
  ...
  {
		Il2CppIntPtr buffer_ptr = {stream->buffer};
		Il2CppIntPtr gchandle_ptr = {stream->gchandle};
...
Il2CppIntPtr CreateZStream(int32_t compress, uint8_t gzip, methodPointerType func_ptr, Il2CppIntPtr gchandle)
{
  z_stream *z;
  int32_t retval;
  ZStream *result;

	Il2CppIntPtr result_ptr = {0};
  ...
  result->stream = z;
  result->func = func;
  result->gchandle = gchandle.m_value;
  result->compress = compress;
  result->buffer = (uint8_t*)malloc(BUFFER_SIZE * sizeof(uint8_t));

	result_ptr.m_value = result;
  return result_ptr;
}

int32_t CloseZStream(Il2CppIntPtr zstream)
{
  int32_t status;
  int32_t flush_status;

	ZStream *stream = (ZStream*)zstream.m_value;
...
int32_t Flush(Il2CppIntPtr zstream)
{
	ZStream *stream = (ZStream*)zstream.m_value;
...
int32_t ReadZStream(Il2CppIntPtr zstream, Il2CppIntPtr zbuffer, int32_t length)
{
  int32_t n;
  int32_t status;
	z_stream *zs;
	...
	ZStream *stream = (ZStream*)zstream.m_value;
	uint8_t *buffer = (uint8_t*)zbuffer.m_value;
...
int32_t WriteZStream(Il2CppIntPtr zstream, Il2CppIntPtr zbuffer, int32_t length)
{
  int32_t n;
  int32_t status;
  z_stream *zs;
